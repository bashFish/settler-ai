####
# this just served as a test/ PoC,
#     finding out how to draw stuff & interact
...
class SnakeAndApple:
    ...
    # ------------------------------------------------------------------
    # Initializing Functions/ Game Loops:
    # ------------------------------------------------------------------
    def __init__(self):
        self.window = Tk()
    ...
    def _cell_to_coordinates(self, cell):
        x1 = cell[0] * row_h
        y1 = cell[1] * col_w
        x2 = x1 + row_h
        y2 = y1 + col_w
    ...
    def place_apple(self):
        ...
        self.apple_obj = self.canvas.create_rectangle(
            self._cell_to_coordinates(self.apple_cell), fill=RED_COLOR_LIGHT, outline=BLUE_COLOR,
        )
    ...
    def display_snake(self, mode=""):
        ...
        if mode == "complete":
            for i, cell in enumerate(self.snake):
                ...
                self.snake_objects.append(
                    self.canvas.create_rectangle(
                        self._cell_to_coordinates(cell), fill=BLUE_COLOR, outline=RED_COLOR,
                    )
                )
        else:
            # only update head
            ...
            #TODO: bug if apples eaten too fast
            if self.snake[0] == self.old_apple_cell:
                ...
                self.snake_objects.insert(
                    0,
                    self.canvas.create_rectangle(
                        self._cell_to_coordinates(tail), fill=BLUE_COLOR, outline=RED_COLOR
                    ),
                )
            self.window.update()
